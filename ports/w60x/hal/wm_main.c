...
int main(void) {
    ...
    /* Check, whether the bootloader has to be started */

    u32 magic_value = *(u32 *)BOOTLOADER_MAGIC_ADDRESS;
    *(u32 *)BOOTLOADER_MAGIC_ADDRESS = 0;

    if (magic_value == BOOTLOADER_MAGIC_NUMBER) {
        typedef void (*rom_func_dlfw)(int, int);
        rom_func_dlfw enter_rom_dlfw = (rom_func_dlfw)ROM_FIRMWARE_LOADER_ADDRESS;
...
void task_start(void *data) {
    ...
    if (tls_wifi_mem_cfg(WIFI_MEM_START_ADDR, 7, 7)) {   /*wifi tx&rx mem customized interface*/
        TLS_DBGPRT_INFO("wl mem initial failed\n");
    }
    ...
    if (tls_wl_init(NULL, &mac_addr[0], NULL) == NULL) {
        TLS_DBGPRT_INFO("wl driver initial failed\n");
    }
    if (wpa_supplicant_init(mac_addr)) {
        TLS_DBGPRT_INFO("supplicant initial failed\n");
    }
