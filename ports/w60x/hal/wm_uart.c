 ...
 * @retval
 *
 * @note This function should be called after the function tls_uart_port_init() or it won't work.
 */
...
*
* Returns    :
* Notes      :	If user want to resume uart old function for AT command,must call this function.
**********************************************************************************************************/
#if 0
...
/**
 * @brief	   This function is used to transfer data through DMA.
 *
...
int tls_uart_dma_write(char *buf, u16 writesize, void (*cmpl_callback)(void *p), u16 uart_no) {
    ...
    if (port->tx_dma_on) {
        TLS_DBGPRT_ERR("transmitting, wait\n");
        return WM_FAILED;
    }
